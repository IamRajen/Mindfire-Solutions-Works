<cfset local.searchQuery = ''/>
<cfif structKeyExists(url, "query")>
    <cfset local.searchQuery = url.QUERY/>
</cfif>
...
<form class="form-inline my-1 my-lg-0 px-2 py-3" method="get" action="searchResult.cfm">
    <div class="text-center d-block w-100 ">
        ...
        aria-label="Search" value="<cfoutput>#local.searchQuery#</cfoutput>"><input src="Images/searchLogo.png" class="btn border-secondary border-left-0 my-2 search-button" type="image">
    </div> 
    <div class='px-5 w-100 text-center my-1'>
        <cfset local.trendingWords = local.batchServiceObj.getTrendingWord()/>
        <small class='text-dark text-center d-inline p-2'><i><b>Trending: </b></i></small>
        <cfif isQuery(local.trendingWords.words)>
            <cfoutput query="local.trendingWords.words">
